...
def bin_to_dec(binary_string):
    ...
    power = binary_string_len - 1
    count = 0
    decimal = 0
    while count < binary_string_len:
        digit = int(binary_string[count])
...
def dec_to_bin(number):
    ...
    power = 0
    one_zero = 0
    test_difference = number_int
    while test_difference > 0:
        test_difference -= (2 ** power)
    ...
    while number_int > 0 or power >(-1):
        number_int -= 2 ** power
        if (number_int) > 0:
            power -= 1
            odd_and_even_list.append("1")
        elif (number_int) == 0:
            power -= 1
            if one_zero == 0:
                odd_and_even_list.append("1")
...
def telephone_cipher(message):
    ...
    for character in message_list:
        numerical_equivalent = str(encoder_dict[character])
        if count > 0 and numerical_list[-1][-1] in numerical_equivalent:
            numerical_list.append("_")
        numerical_list.append(numerical_equivalent)
        count += 1
    numerical = ''.join(numerical_list)
    return numerical
...
def telephone_decipher(telephone_string):
    ...
    telephone_str = telephone_string
    first_num = 0
    second_num = 1
    count = 0
    message_list = []
    ...
    while second_num < len(telephone_str):
        if telephone_str[first_num] == telephone_str[second_num]:
            first_num += 1
            second_num += 1
        if telephone_str[second_num] == "Z":
            multiplier = second_num-count
            num_letter = telephone_str[first_num] * multiplier
            message_list.append(decipher_dict[num_letter])
            break
        elif telephone_str[first_num] != telephone_str[second_num]:
            multiplier = second_num-count
            num_letter = telephone_str[first_num] * multiplier
            message_list.append(decipher_dict[num_letter])
            num_letter = " "
